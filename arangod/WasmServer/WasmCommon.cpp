...
auto arangodb::wasm::wasmFunction2Velocypack(WasmFunction const& wasmFunction)
    -> Slice {
  VPackBuilder builder;
  {
    auto ob = VPackObjectBuilder(&builder);
    builder.add("name", VPackValue(wasmFunction.name));
    builder.add("code", arangodb::wasm::code2Velocypack(wasmFunction.code));
    builder.add("isDeterministic", VPackValue(wasmFunction.isDeterministic));
  }
  return builder.slice();
}

auto arangodb::wasm::code2Velocypack(Code const& code) -> Slice {
  VPackBuilder builder;
  {
